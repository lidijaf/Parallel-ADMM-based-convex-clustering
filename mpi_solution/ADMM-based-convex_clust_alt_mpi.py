...
def admm_kmeans(filename, folder, workers=5, num_iter=100, lmbd=1, rho=1, eps=0.1, abstol = 1e-4, reltol = 1e-2):
    ...
    for i in range(num_iter):
      x_local = update_x_all(rho, workers, lmbd, d, chunk_size, mu, L, inner_iters, i, y, lambdaVal, a_local, x_local, rank)
...
def update_x_all(rho, workers, lmbd, d, chunk_size, mu, L, inner_iters, outer, y, lambdaVal, a, x, rank):
    if rank==0:
        return update_x_zero(a, x, y, lmbd, workers, rho, d, chunk_size, mu, L, inner_iters, outer)
    else:
        return update_x(rho, lmbd, d, chunk_size, mu, L, inner_iters, outer, a , x, y[rank-1], lambdaVal[rank-1], rank)

def update_x(rho, lmbd, d, chunk_size, mu, L, inner_iters, outer, a, x, y, lambdaVal, rank):
    limit = 0.1/(outer+1)
    z = np.copy(x)
